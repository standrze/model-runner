...
AUTORELEASE_PATCH="$ROOT/Patches/mlx-swift-lm-generation-loop-autorelease.patch"
...
AUTORELEASE_CALL='apply_dependency_patch "mlx-swift-lm generation-loop autorelease fast path" "$MLX_SWIFT_LM_CHECKOUT" "$MLX_SWIFT_LM_GENERATION_LOOP_AUTORELEASE_PATCH"'
...
for statement in \
  "$PATCH_DECL" "$REVISION_DECL" "$VERIFY_CALL" "$AUTORELEASE_CALL" "$PATCH_CALL"; do
...
AUTORELEASE_LINE="$(grep -Fn "$AUTORELEASE_CALL" "$PREPARE_FLAT" | cut -d: -f1)"
...
if [[ "$PATCH_DECL_LINE" -ge "$PATCH_CALL_LINE" ]] \
  ...
  || [[ "$VERIFY_LINE" -ge "$PATCH_CALL_LINE" ]] \
  || [[ "$AUTORELEASE_LINE" -ge "$PATCH_CALL_LINE" ]]; then
  echo "Pinned revision and prerequisite patch must precede the decoder patch" >&2
  exit 1
fi
...
# Match the real preparation order and prove that the two patches compose.
git -C "$TEST_ROOT" apply --check --whitespace=error-all "$AUTORELEASE_PATCH"
git -C "$TEST_ROOT" apply "$AUTORELEASE_PATCH"
...
git -C "$TEST_ROOT" apply --reverse --check "$AUTORELEASE_PATCH"
git -C "$TEST_ROOT" apply --reverse "$AUTORELEASE_PATCH"
